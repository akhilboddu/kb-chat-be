...
def extract_json_from_markdown(markdown_string: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "error": error_msg,
            "original_content": markdown_string,
        }  # Return error dict
...
default_config = {
    ...
    "REQUIRED_FIELDS": [
        "business_name",
        "short_description",
        "payment_information.payment_methods",
        "payment_information.payment_plans",
    ],
    "MIN_PRODUCTS": 2,
    "MIN_SERVICES": 1,
    "MIN_FAQS": 3,
...
def load_scraper_config(config_path="scraper_config.json") -> Dict[str, Any]:
    ...
    try:
        ...
        if os.path.exists(abs_config_path):
            ...
            logger.warning(
                f"Configuration file not found at {abs_config_path}. Using default settings."
            )
    except json.JSONDecodeError as e:
        logger.error(
            f"Error decoding JSON from {config_path}: {e}. Using default settings."
        )
        loaded_config = {}
    except Exception as e:
        logger.error(
            f"Error loading configuration file {config_path}: {e}. Using default settings."
        )
        loaded_config = {}
...
def is_sufficient_data(profile: Dict[str, Any]) -> bool:
    ...
    if len(profile.get("offerings", [])) < config.get("MIN_PRODUCTS", 2) + config.get(
        "MIN_SERVICES", 1
    ):  # Simplified check
        return False

    offerings_with_pricing = sum(
        1 for offering in profile.get("offerings", []) if offering.get("pricing")
    )
    ...
    if (
        not payment_options.get("methods")
        or len(payment_options.get("methods", [])) < 2
        or not payment_options.get("plans")
        or len(payment_options.get("plans", [])) < 2
    ):
        return False
    ...
    if (
        not profile.get("value_props")
        or len(profile.get("value_props", [])) < 3
        or not profile.get("audience")
        or len(profile.get("audience", [])) < 3
    ):
        return False
...
class Scraper:
    def __init__(self):
        ...
        llm_mode = os.getenv(
            "LLM_MODE", "google"
        ).lower()  # Default to google if not set
        ...
        if llm_mode == "google":
            ...
            if google_api_key:
                ...
                logger.warning(
                    "LLM_MODE set to 'google' but GOOGLE_API_KEY is not found in .env."
                )
            ...
            if deepseek_api_key:
                ...
                logger.warning(
                    "LLM_MODE set to 'deepseek' but DEEPSEEK_API_KEY is not found in .env."
                )
        else:
            logger.warning(
                f"Invalid LLM_MODE specified: '{llm_mode}'. Please use 'google' or 'deepseek'. Falling back."
            )
        ...
        if not self.llm:
            raise ValueError(
                f"LLM could not be configured. Check LLM_MODE ('{llm_mode}') and ensure the corresponding API key (GOOGLE_API_KEY or DEEPSEEK_API_KEY) is set in your .env file."
            )
    ...
    async def _extract_focused_content(self, page: Page) -> str:
        ...
        if not text_content.strip():
            logger.warning(
                "No targeted content found, falling back to full body text (might be less accurate)."
            )
            try:
    ...
    async def _extract_social_links(
        self, page: Page, current_url: str
    ) -> Dict[str, str]:
            ...
            try:
                ...
                if elements:
                    ...
                    if href:
                        ...
                        if (
                            parsed_social.scheme in ["http", "https"]
                            and parsed_social.netloc
                        ):
    ...
    async def _extract_internal_links(self, page: Page, current_url: str) -> Set[str]:
                        ...
                        try:
                            ...
                            if href and not href.startswith(
                                ...
                                if parsed_url.netloc == urlparse(
                                    ...
                                    logger.info(
                                        f"Found potential course/product link: {normalized_url}"
                                    )
                        except Exception as e:
                            pass
            except Exception as e:
                            ...
                            for i in range(count):
                                try:
                                    ...
                                    if href and not href.startswith(
                                        ...
                                        if parsed_url.netloc == urlparse(
                                            base_url
                                        ).netloc and parsed_url.scheme in [
                                            "http",
                                            "https",
                                        ]:
                                            # Remove hash fragments and normalize
                                            normalized_url = abs_url.split("#")[
                                                0
                                            ].rstrip("/")
                                            internal_links.add(normalized_url)
                                except Exception as e:
        ...
        sorted_links = sorted(
            ...
            key=lambda url: 0
            if any(
                re.search(pattern, urlparse(url).path) for pattern in priority_patterns
            )
            else 1,
        )
        ...
        # Log the found links
        logger.info(
            f"Found {len(sorted_links)} relevant internal links (filtered from {len(internal_links)} total) on {current_url}"
        )
    ...
    async def _extract_pricing_elements(self, page: Page) -> Dict[str, str]:
        ...
        currency_pattern_text_selector = (
            rf"/:has-text(/(?:{currency_symbols_escaped})\s?\d/)"
        )
                    ...
                    for i in range(count):
                        try:
                            ...
                            for price_match in re.finditer(
                                currency_pattern_regex, cleaned_text
                            ):
                                # Combine symbol and number group to get the full price string
                                extracted_price = price_match.group(
                                    1
                                ) + price_match.group(2)
                                ...
                                if (
                                    "datascience" in url_path
                                    or "data-science" in url_path
                                ):
                                    course_name = "Data Science Bootcamp"
                                elif (
                                    "fullstack" in url_path
                                    or "web-development" in url_path
                                ):
                                        ...
                                        for h_selector in heading_selectors:
                                            try:
                                                heading = parent.locator(
                                                    f"xpath=ancestor-or-self::*/*[{h_selector}]"
                                                ).first
                                                if await heading.count() > 0:
                                                    heading_text = (
                                                        await heading.text_content()
                                                    )
                                                    if heading_text:
                                                        course_name = (
                                                            heading_text.strip()
                                                ...
                                                pass  # Ignore errors finding headings
                                        if found_name:
                                            break  # Stop searching ancestors if name found
                                        try:
                                            ...
                                            break  # Stop if error moving up

                                # Store the price with course name, only if not already found or if this is more specific
                                if course_name not in pricing_info or len(
                                    extracted_price
                                ) > len(pricing_info.get(course_name, "")):
                                    pricing_info[course_name] = extracted_price
                                    logger.info(
                                        f"Found pricing: {extracted_price} for {course_name} using selector: {selector}"
                                    )
                        except Exception as e:
                            logger.debug(
                                f"Error extracting price from element {i} for selector '{selector}': {e}"
                            )
            except Exception as e:
    ...
    async def analyze_page_content_for_sales(
        ...
        if len(content) > self.config.get("MAX_CONTENT_LENGTH", 10000):
            logger.debug(
                f"Truncating content for LLM analysis from {len(content)} to {self.config.get('MAX_CONTENT_LENGTH', 10000)} chars for {url}"
            )
        ...
        try:
            ...
            if "error" in result:
                logger.error(
                    f"LLM analysis failed for {url}. Error: {result.get('error')}"
                )
            ...
            if (
                "products" in result or "services" in result
            ) and "offerings" not in result:
                result["offerings"] = []
            ...
            for field in payment_list_fields:
                if field in result.get("payment_information", {}) and not isinstance(
                    result["payment_information"][field], list
                ):
        ...
        try:
            ...
            try:
                ...
                if not response or not response.ok:
                    logger.warning(
                        f"Failed to load {url}: Status {response.status if response else 'N/A'}"
                    )
                    return {
                        "error": f"Failed to load page: Status {response.status if response else 'N/A'}",
                        "url": url,
                    }
                ...
                if (
                    len(self.scraped_urls) <= 3
                ):  # Only extract from first 3 pages to save time
                    new_internal_links = await self._extract_internal_links(
                        page, current_url
                    )
                    ...
                    self.priority_queue = sorted(
                        ...
                        key=lambda url: 0
                        if any(
                            re.search(pattern, urlparse(url).path)
                            for pattern in self.config.get("PRIORITY_URL_PATTERNS", [])
                        )
                        else 1,
                    )
                ...
                for existing_url, existing_content in self.page_contents.items():
                    ...
                    if similarity > 0.7:  # Threshold for considering pages similar
                        logger.info(
                            f"Skipping page with similar content to {existing_url} (similarity: {similarity:.2f})"
                        )
                        return {
                ...
                analysis_result = await self.analyze_page_content_for_sales(
                    text_content, current_url
                )
                ...
                if pricing_info:
                    ...
                    if analysis_result.get("offerings") and isinstance(
                        analysis_result["offerings"], list
                    ):
                        for offering in analysis_result["offerings"]:
                            if offering.get("name"):
                                ...
                                for course_name, price in pricing_info.items():
                                    if (
                                        course_name.lower() in offering["name"].lower()
                                        or offering["name"].lower()
                                        in course_name.lower()
                                    ):
                                        offering["pricing"] = price
                                        logger.info(
                                            f"Updated pricing for {offering['name']} to {price}"
                                        )
    ...
    def _compile_flexible_business_profile(self, results=None) -> Dict[str, Any]:
        ...
        homepage_result = next(
            (
                page
                for page in results
                if "error" not in page
                and page.get("page_topic", "").lower()
                in ["homepage", "home", "main", "landing"]
            ),
            results[0],
        )
        ...
        for page_result in results:
            if "_direct_pricing" in page_result and isinstance(
                page_result["_direct_pricing"], dict
            ):
        ...
        for page_result in results:
            ...
            if page_result.get("contact_info") and isinstance(
                page_result["contact_info"], dict
            ):
                for key, value in page_result["contact_info"].items():
                    if value and (
                        key not in profile["contact_info"]
                        or not profile["contact_info"][key]
                    ):
                        profile["contact_info"][key] = value

            # Handle old format contact info
            if page_result.get("sales_contact_info") and isinstance(
                page_result["sales_contact_info"], dict
            ):
            ...
            if page_result.get("offerings") and isinstance(
                page_result["offerings"], list
            ):
                for offering in page_result["offerings"]:
                    ...
                    if offering_name in seen_offerings:
                        ...
                        if offering.get("description") and (
                            not existing.get("description")
                            or len(str(offering["description"]))
                            > len(str(existing["description"]))
                        ):
                        ...
                        if offering.get("attributes"):
                            existing_attrs = {
                                attr.lower() for attr in existing.get("attributes", [])
                            }
            ...
            if page_result.get("payment_information") and isinstance(
                page_result["payment_information"], dict
            ):
                ...
                # Free offers/trials
                free_offer = payment_info.get("free_offers") or payment_info.get(
                    "free_trials"
                )
                if free_offer and (
                    not profile["payment_options"]["free_offer"]
                    or len(str(free_offer))
                    > len(str(profile["payment_options"]["free_offer"] or ""))
                ):
                    profile["payment_options"]["free_offer"] = free_offer

            # Process value propositions (both new and old format)
            value_props_field = page_result.get(
                "value_propositions", []
            ) or page_result.get("unique_selling_proposition_usp", [])
            ...
            audience_field = page_result.get("target_audience", []) or page_result.get(
                "customer_segments", []
            )
        ...
        if direct_pricing:
            profile["course_prices"] = (
                direct_pricing  # Store the raw directly extracted prices
            )
            for offering in profile.get("offerings", []):
                if offering.get("name"):
                    offering_name_lower = offering["name"].lower()
                    best_match_price = offering.get(
                        "pricing"
                    ...
                    for course_key, price in direct_pricing.items():
                        ...
                        if (
                            offering_name_lower == course_key_lower
                            or offering_name_lower in course_key_lower
                            or course_key_lower in offering_name_lower
                            or
                            # Simple keyword check (e.g., 'datascience' vs 'data science bootcamp')
                            any(
                                keyword in course_key_lower
                                for keyword in offering_name_lower.split()
                                if len(keyword) > 3
                            )
                            or any(
                                keyword in offering_name_lower
                                for keyword in course_key_lower.split()
                                if len(keyword) > 3
                            )
                        ):
                            # Prioritize updating if current price is None or if this price seems more specific
                            if best_match_price is None or (
                                price and len(str(price)) > len(str(best_match_price))
                            ):
                                best_match_price = price
                                logger.info(
                                    f"Applying direct pricing match: {price} to {offering['name']}"
                                )
        ...
        if profile["offerings"]:
            product_count = sum(
                1 for o in profile["offerings"] if o.get("type") == "product"
            )
            service_count = sum(
                1 for o in profile["offerings"] if o.get("type") == "service"
            )
        ...
        logger.info(
            f"Compiled business profile with {len(profile['offerings'])} offerings, "
    ...
    async def scrape(self, url: str, max_pages: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.max_pages = (
                ...
                else self.config.get("MAX_INTERNAL_PAGES", 15)
            )
            ...
            while (
                (self.priority_queue or self.internal_links)
                and len(self.scraped_urls) < self.max_pages
                and not self.early_termination
            ):
                # Get next batch of URLs to process
                if self.priority_queue:
                    urls_to_process = self.priority_queue[
                        : self.max_pages - len(self.scraped_urls)
                    ]
                    self.priority_queue = self.priority_queue[len(urls_to_process) :]
                else:
                    urls_to_process = list(self.internal_links - self.scraped_urls)[
                        : self.max_pages - len(self.scraped_urls)
                    ]
                ...
                batch_size = min(
                    self.config.get("MAX_CONCURRENT_SCRAPES", 5), len(urls_to_process)
                )
                for i in range(0, len(urls_to_process), batch_size):
                    if self.early_termination:
                        logger.info(
                            "Early termination triggered - stopping batch processing"
                        )
                        break
                    ...
                    if self.early_termination:
                        logger.info(
                            "Early termination condition met - stopping batch processing"
                        )
                        break
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Web scraper for business profiles with multi-page support"
    )
    ...
    if len(sys.argv) == 1:
        ...
        example_max_pages = config.get(
            "MAX_INTERNAL_PAGES", 10
        )  # Reduced example default
        print(
            f"Running with default example: python -m src.crawler.scraper https://www.zaio.io/ --max-pages {example_max_pages}"
        )
    ...
    if (
        results
        and "business_profile" in results
        and "error" not in results.get("business_profile", {})
    ):
        print("\n--- Scrape Completed Successfully ---")
        print(f"URL: {results.get('scrape_metadata', {}).get('url', 'N/A')}")
        print(
            f"Pages Scraped: {results.get('scrape_metadata', {}).get('pages_scraped', 'N/A')}"
        )
        print(
            f"Time: {results.get('scrape_metadata', {}).get('processing_time_seconds', 'N/A'):.2f} seconds"
        )
